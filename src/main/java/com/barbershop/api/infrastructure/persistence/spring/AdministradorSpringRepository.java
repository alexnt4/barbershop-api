package com.barbershop.api.infrastructure.persistence.spring;

import org.springframework.data.mongodb.repository.MongoRepository;
import org.springframework.stereotype.Repository;

import com.barbershop.api.infrastructure.persistence.mongodb.documents.AdministradorDocument;

/**
 * Adaptador de Spring Data MongoDB para Administrador.
 * Proporciona métodos CRUD automáticos y finder methods personalizados.
 */
@Repository
public interface AdministradorSpringRepository extends MongoRepository<AdministradorDocument, String> {

    /**
     * Busca un administrador por su email.
     *
     * @param email correo electrónico
     * @return documento si existe
     */
    AdministradorDocument findByEmail(String email);
}
